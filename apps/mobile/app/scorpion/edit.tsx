/**
 * Edit scorpion form — Phase 3b.
 *
 * Reads `?id=` from the route query (matches the /tarantula/edit
 * convention). Prefills from getScorpion, PUTs on save. Husbandry
...
const ENCLOSURE_OPTIONS: { value: ScorpionEnclosureType; label: string }[] = [
  { value: 'terrestrial', label: 'Terrestrial' },
  { value: 'arboreal', label: 'Arboreal' },
  { value: 'fossorial', label: 'Fossorial' },
];

export default function EditScorpionScreen() {
  ...
  const [form, setForm] = useState<Scorpion | null>(null);
  ...
  const update = <K extends keyof Scorpion>(key: K, value: Scorpion[K]) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await updateScorpion(id, payload);
      router.back();
    } catch (err) {
      Alert.alert(
        'Could not save',
