/**
 * Generic invert edit form — ADR-007. Resolves taxon from the record,
 * registry supplies size label + default enclosure type. PUT /inverts/{id}.
 */
import React, { useEffect, useState } from 'react';
import {
  ActivityIndicator, Alert, KeyboardAvoidingView, Platform, ScrollView, StyleSheet, Switch, Text, TextInput, TouchableOpacity, View,
} from 'react-native';
...
export default function EditInvertScreen() {
  ...
  if (loading || !form) return <View style={[styles.flex, styles.center]}><ActivityIndicator color={colors.primary} size="large" /></View>;
  const meta = INVERT_TAXA[form.taxon];
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.row}>
            ...
            <Field label="Humidity max %" flex><TextInput style={styles.input} value={form.target_humidity_max ?? ''} onChangeText={(t) => update('target_humidity_max', t)} keyboardType="decimal-pad" /></Field>
          </View>
  ...
  return (
    <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
      {options.map((opt) => {
        ...
        return (
          <TouchableOpacity key={opt.value} onPress={() => onChange(opt.value)} style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, borderWidth: 1, borderColor: selected ? colors.primary : colors.border, backgroundColor: selected ? colors.primary : colors.surface }}>
