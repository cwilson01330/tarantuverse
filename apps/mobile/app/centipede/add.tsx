...
export default function AddCentipedeScreen() {
  ...
  const [legPairCount, setLegPairCount] = useState('');
  ...
  const handleSave = async () => {
    if (!name && !commonName && !scientificName) {
      Alert.alert(
        'Add an identifier',
        'Pick a species or give your centipede a name before saving.',
      );
      return;
    }

    const payload: CentipedeCreate = {
      name: name.trim() || null,
      ...
      current_segment_count: segmentCount ? Number(segmentCount) : null,
      current_leg_pair_count: legPairCount ? Number(legPairCount) : null,
      ...
      >
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <Field label="Common name (override)">
            <TextInput
              style={styles.input}
              placeholder="Giant Desert Centipede"
              placeholderTextColor={colors.textTertiary}
              value={commonName}
