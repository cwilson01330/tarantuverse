/**
 * Log substrate change for a centipede — Phase 3b.
 *
 * Mirrors the tarantula substrate flow. The backend auto-denormalizes
 * the change date + type + depth onto the parent centipede (see
 * apps/api/app/routers/substrate_changes.py), so the detail screen
 * shows "last refreshed: X days ago" without a second fetch.
 */
...
import { createCentipedeSubstrateChange } from '../../src/lib/centipedes';
...
export default function AddCentipedeSubstrateChangeScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await createCentipedeSubstrateChange(id, {
        // Substrate changes use a date column (not datetime), so the
        // wire value is YYYY-MM-DD directly. No T12:00:00 hack needed.
        changed_at: date,
