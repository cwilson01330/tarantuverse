/**
 * Centipede species care sheet — Phase 3b.
 *
 * Renders one row from the Phase 2 catalog as a care sheet. Mirrors
 * the structure of the tarantula care sheet but with centipede-
...
function CentipedeSpeciesCareSheetScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();
  const { colors, layout } = useTheme();
  const iconColor = layout.useGradient ? '#fff' : colors.textPrimary;

  const [species, setSpecies] = useState<CentipedeSpecies | null>(null);
  ...
  const fetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const s = await getCentipedeSpecies(id);
      setSpecies(s);
    } catch (err) {
      setError(
  ...
  const developmentalClassExplainer =
    species.developmental_class === 'epimorphic'
      ? 'Hatches with adult segment count.'
      : species.developmental_class === 'anamorphic'
        ? 'Adds segments with each molt.'
        : null;

  const hasBiology =
    species.developmental_class != null
  ...
  return (
    <View style={styles.flex}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {/* Biology — centipede-specific developmental + morphology facts. */}
        {hasBiology && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Biology</Text>
            {species.developmental_class && (
              <FactRow
                label="Developmental class"
                value={`${cap(species.developmental_class)}${
                  developmentalClassExplainer
                    ? ` — ${developmentalClassExplainer}`
                    : ''
                }`}
                colors={colors}
            ...
            {species.typical_segment_count != null && (
              <FactRow
                label="Typical segments"
                value={`${species.typical_segment_count} body segments`}
                colors={colors}
              />
            )}
            {species.typical_leg_pair_count != null && (
              <FactRow
                label="Typical leg pairs"
                value={`${species.typical_leg_pair_count} leg pairs`}
                colors={colors}
              />
            )}
          </View>
...
export default withErrorBoundary(CentipedeSpeciesCareSheetScreen);
