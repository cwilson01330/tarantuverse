/**
 * Colony: add photo — cph_20260729_colony_logs.
 *
 * A communal tarantula is a display animal housed as a group, so it gets the
 * same gallery flow as any individual. Deliberately a sibling of
 * app/invert/add-photo.tsx rather than a generalisation of it: that screen
 * resolves a taxon before uploading (its routes are per-taxon facades) and a
 * colony has no equivalent step, so merging them would mean a branch in the
...
export default function AddColonyPhotoScreen() {
  ...
  const handleUpload = async () => {
    if (!id || !imageUri) { if (!imageUri) Alert.alert('Pick a photo first.'); return; }
    try {
      setUploading(true);
      ...
      await uploadColonyPhoto(id, form);
      router.back();
    } catch (err: any) {
  ...
  return (
    <View style={styles.flex}>
      <AppHeader title="Add photo"
        ...
        rightAction={<TouchableOpacity onPress={handleUpload} disabled={uploading || !imageUri} style={{ opacity: uploading || !imageUri ? 0.4 : 1 }}>{uploading ? <ActivityIndicator color={iconColor} size="small" /> : <Text style={{ color: iconColor, fontSize: 16, fontWeight: '600' }}>Upload</Text>}</TouchableOpacity>}
      />
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.captionWrap}>
          ...
          <TextInput style={styles.captionInput} value={caption} onChangeText={setCaption} placeholder="Group shot, new setup, molt found, etc." placeholderTextColor={colors.textTertiary} multiline maxLength={500} />
        </View>
      </ScrollView>
