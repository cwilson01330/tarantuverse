/**
 * Log feeding for a whip spider — ADR-006 taxon #1.
 *
 * Slim log-feeding screen, mirror of the centipede version. Whip
 * spiders are predators (live prey), so the food-type chips list the
 * usual feeders.
 */
...
import { createWhipSpiderFeeding } from '../../src/lib/whip-spiders';
...
export default function AddWhipSpiderFeedingScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      // Convert the YYYY-MM-DD picker value into a midday-local ISO so
      // the keeper sees the same date they typed (the calendar-day memory).
      const fedAt = new Date(date + 'T12:00:00').toISOString();
      await createWhipSpiderFeeding(id, {
        fed_at: fedAt,
        food_type: foodType,
        accepted,
