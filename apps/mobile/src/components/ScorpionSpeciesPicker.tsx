/**
 * Scorpion species picker — debounced autocomplete used by the
 * add/edit forms.
 *
 * Reuses the catalog API from src/lib/scorpions.ts. The result list
...
interface Props {
  ...
  onChange: (picked: ScorpionSpecies | null) => void;
  placeholder?: string;
}

const DEBOUNCE_MS = 250;

export function ScorpionSpeciesPicker({
  valueId,
  valueScientific,
  onChange,
  ...
  const [results, setResults] = useState<ScorpionSpecies[]>([]);
  ...
  const handleChangeText = (text: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const rows = await searchScorpionSpecies(text.trim(), 8);
        setResults(rows);
      } catch {
        setResults([]);
      } finally {
  ...
  const handlePick = (row: ScorpionSpecies) => {
    onChange(row);
