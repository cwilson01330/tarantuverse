 ...
 *
 * OBSERVED ONLY — there is no forecast here and there must not be one. Isopod
 * reproduction depends on species, temperature, humidity, calcium, protein,
 * substrate depth and founding sex ratio, and that last one is unknowable:
 * most isopods can't be sexed at a glance and nobody counts a colony living
 * inside substrate. `count_is_estimated` defaults to true for exactly that
 * reason, so a projection would be a guess stacked on a guess.
 *
...
export interface PopulationHistory {
  ...
  /** False when counts were written outside the event log — say so rather
   *  than drawing a line that disagrees with the headline number. */
  history_complete: boolean;
  count_is_estimated: boolean;
  growth: PopulationGrowth;
}

export async function getColonyPopulationHistory(id: string): Promise<PopulationHistory> {
  const { data } = await apiClient.get<PopulationHistory>(`/colonies/${id}/population-history`);
  return data;
}
