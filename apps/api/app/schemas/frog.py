...
Mirror of LizardBase/Create/Update/Response. Same husbandry shape
because all three HV taxa share the same per-animal record: name,
species link, enclosure link, sex, acquisition info, current
measurements, feeding schedule phrase, denormalized last_fed_at /
last_shed_at, brumation flags, privacy, notes.
"""
...
class FrogBase(BaseModel):
    """Fields a keeper can set when creating/updating a frog."""
    ...
    # amphibians; see Frog model docstring.
...
class FrogCreate(FrogBase):
    """Schema for creating a new frog."""
    reptile_species_id: Optional[uuid.UUID] = None
    enclosure_id: Optional[uuid.UUID] = None


class FrogUpdate(FrogBase):
    """Schema for updating a frog. Inherits all-optional from FrogBase."""
    reptile_species_id: Optional[uuid.UUID] = None
    enclosure_id: Optional[uuid.UUID] = None
