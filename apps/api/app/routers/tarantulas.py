...
# NB: `GET /{tarantula_id}/premolt-prediction` was removed here (ADR-013
# follow-up, 2026-07-28). It was a SECOND premolt algorithm — additive
# "probability" points over feeding/molt history — living alongside the
# canonical `premolt_service` (refusal streak + molt interval). The two could
# reach different conclusions about the same animal, so the collection grid
# and the dashboard could disagree, and neither number was calibrated against
# recorded molt outcomes.
#
# All callers now use `/premolt/dashboard` (batch) or
# `/premolt/tarantulas/{id}/prediction` (single). Deleted rather than
# deprecated because every caller is in this repo and was migrated in the same
# change — there is no external consumer to break.
