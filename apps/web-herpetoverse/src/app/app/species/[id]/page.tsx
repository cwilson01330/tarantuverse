...
export default async function SpeciesDetailPage({ params }: PageProps) {
  ...
  return (
    <article className="max-w-4xl mx-auto">
      ...
      {(species.uvb_required || species.uvb_type === 'T5_HO' || species.uvb_type === 'T8') && (
...
function UvbBlock({ species }: { species: ReptileSpecies }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      <LabeledStat
        label="Required?"
        value={species.uvb_required ? 'Yes — provide UVB' : 'Optional / beneficial'}
      />
...
function WaterBehaviorBlock({ species }: { species: ReptileSpecies }) {
  ...
  if (!hasAny) {
    return (
      <p className="text-neutral-500">No behavioral notes recorded yet.</p>
    )
  }
